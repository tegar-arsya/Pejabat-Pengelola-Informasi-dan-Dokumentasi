...
if ($_SERVER["REQUEST_METHOD"] == "POST" && verify_csrf_token($_POST['csrf_token'])) {
    ...
    if (!validate_email($email)) {
        ...
        echo "<script>alert('" . htmlspecialchars($_SESSION['login_error']) . "'); window.location.href='../../../';</script>";
        exit();
    }
    ...
    if ($sql = $conn->prepare("SELECT r.id, r.email, r.password, r.nama_depan, r.nama_belakang, r.nik
                           FROM registrasi r
        ...
        $_SESSION['login_error'] = "Error preparing the statement.";
        echo "<script>alert('" . htmlspecialchars($_SESSION['login_error']) . "'); window.location.href='../../../';</script>";
        exit();
    }
    $conn->close();
