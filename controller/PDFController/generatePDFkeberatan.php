            ...
            if ($mail->send()) {
                ...
                echo "<script>alert('Gagal mengirim email. Silakan coba lagi.');</script>";
                    ...
                    if ($mailOPD->send()) {
                        ...
                        $insertNotifikasi = "INSERT INTO notifikasi_pengiriman_keberatan (id_permohonan_keberatan, nomer_registrasi_keberatan, status)
                            VALUES ($idPermohonan,'{$row['nomer_registrasi_keberatan']}','Permohonan Keberatan Informasi telah dikirim ke {$row['opd_yang_dituju']}')";
