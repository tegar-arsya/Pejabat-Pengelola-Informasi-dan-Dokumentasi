...
if (isset($_POST['id'])) {
    ...
    $query = "SELECT p.id_user, p.nomer_registrasi, r.nik, r.no_hp, r.foto_ktp, r.alamat, r.email, r.nama_depan, r.nama_belakang, r.kota_kabupaten, r.provinsi, r.negara, r.kode_pos, r.pekerjaan, p.opd_yang_dituju, p.informasi_yang_dibutuhkan, p.alasan_pengguna_informasi, p.cara_mendapatkan_informasi, p.cara_mendapatkan_salinan, p.tanggal_permohonan
        FROM permohonan_informasi p
        JOIN registrasi r ON p.id_user = r.nik
        WHERE p.id_user = r.nik AND p.id = $idPermohonan";
