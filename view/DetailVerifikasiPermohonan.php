...
$queryJawaban = "SELECT * FROM answer_admin WHERE nomer_registrasi = '$nomer_registrasi'";
$resultJawaban = $conn->query($queryJawaban);

if ($resultJawaban -> num_rows > 0) {
    while ($row = $resultJawaban -> fetch_assoc()) {
...
$queryPermohonan = "SELECT * FROM permohonan_informasi WHERE nomer_registrasi = '$nomer_registrasi'";
$resultPermohonan = $conn->query($queryPermohonan);

if ($resultPermohonan->num_rows > 0) {
    while ($row = $resultPermohonan->fetch_assoc()) {
        $nama = $row['nama_pengguna']; 
        $tgl =$row['tanggal_permohonan'];
        $tglVerifikasi =$row['tanggal_verifikasi'];
        $nomer_registrasi =$row['kode_permohonan_informasi'];
        $nik =$row['nik'];
        $email =$row['email_pemohon'];
        $foto =$row['foto_ktp'];
        $alamat = $row['alamat'];
        $opd =$row['opd_yang_dituju'];
        $inf =$row['informasi_yang_dibutuhkan'];
    ...
    <?php include '../components/navbarAdmin.php'; ?>
        <div class="content-body">
            <div class="container-fluid">
                ...
                <div class="col-12">
                        <div class="card">
                            <h1>Detail Permohonan Informasi</h1>
                            <div class="card-body">
