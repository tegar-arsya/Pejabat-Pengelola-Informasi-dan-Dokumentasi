                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="permohonanTable" class="table table-striped table-bordered ">
                                        ...
                                        <tbody>
                                            <?php
                                            $query = "SELECT DISTINCT vk.nomer_registrasi_keberatan, vk.id_permohonan_keberatan, pk.tanggal_permohonan, vk.tanggal_verifikasi, sk.tanggal_survey, pk.id, pk.nama_pemohon, pk.alasan_keberatan, pk.opd_yang_dituju, tp.id_permohonan_keberatan AS tp_id_permohonan_keberatan, tp.note, tp.tanggal_penolakan, pk.tanggal_permohonan
                                            ...
                                            if ($result->num_rows > 0) {
                                                ...
                                                while ($row = $result->fetch_assoc()) {
                                                    ...
                                                    echo "<a href='../DetailPermohonan/detailNote?id=" . urlencode($row['id_permohonan_keberatan']) . "' class='btn btn-info btn-sm fas fa-info-circle'></a>";
                                                    echo "<button class='btn btn-danger btn-sm fas fa-trash-alt' onclick='HapusVerifikasi(\"" . htmlspecialchars($row['nomer_registrasi_keberatan']) . "\")'></button>";
                                                    echo "<a href='../FormAnswer/formAnswerKeberatan?registrasi=" . urlencode($row['id_permohonan_keberatan']) . "' class='btn btn-success btn-sm fas fa-reply'></a>";
                                                    echo "<a href='../Form/Note?id=" . urlencode($row['id_permohonan_keberatan']) . "' class='btn btn-primary btn-sm'><i class='fas fa-sticky-note'></i></a>";
                                                    echo "</div>";
                                                    echo "</td>";
                                                    ...
                                                    // Update status in a secure way
                                                    $updateStatusQuery = "UPDATE verifikasi_keberatan SET status = ? WHERE id_permohonan_keberatan = ?";
                                                    $stmt = $conn->prepare($updateStatusQuery);
                                                    $stmt->bind_param('si', $statusDisplay, $row['id_permohonan_keberatan']);
                                                    $stmt->execute();
                                                    ...
                                                    // Check if there is an answer in keberatananswer_admin table
